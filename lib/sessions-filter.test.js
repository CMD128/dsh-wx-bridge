...
function makeBridge(ctx, config = {}) {
  const sent = []
  ...
  const bridge = new SessionBridge(ctx, { push: {}, ...config }, channel, auth, { info() {}, warn() {} })
  bridge.start()
...
test('配置的隐藏工作区会话不列出', async () => {
  const home = makeHome([])
  const records = [
    { header: { id: 'main-1', cwd: '/home/user/workspace-main', delegationDepth: 0 }, title: '主工作区' },
    { header: { id: 'qq-1', cwd: '/home/user/other-agent/state/agents', delegationDepth: 0 }, title: '其他agent' },
    { header: { id: 'main-2', cwd: '/home/user/dsh-wechat', delegationDepth: 0 }, title: '其他工作区' },
  ]
  const { ctx } = makeCtx({ roots: [], records })
  process.env.DSH_HOME = home
  const { bridge, channel } = makeBridge(ctx, { hiddenWorkspacePatterns: ['other-agent'] })
  await bridge.handleInbound({ windowKey: 'user:u-1', text: '/sessions' })
  const out = channel.sent[0].t
  assert.ok(out.includes('主工作区'))
  assert.ok(!out.includes('其他agent'))
